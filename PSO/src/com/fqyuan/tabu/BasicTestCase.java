...
public class BasicTestCase {
	...
	public void runAlgorithm() {
		...
		for (int s = 5; s <= 10; s++) { // the size of the tabu list
			for (double i = 0.5; i <= 2; i += 0.5) { // the amount of iterations
														// (50%, 100%, 150% and
														// 200%)
				Integer maxIterations = new Double(solutions.size() * i).intValue();

				System.out.println(
						String.format("Running TS with tabu list size [%s] and [%s] iterations. Instance size [%s]", s,
								maxIterations, solutions.size()));
				...
				System.out.println(String.format(
						"The algorithm returned a result with [%s] units of distance of the best solution",
						returnedValueIndex));
			}
		}
	}
	...
	public TabuSearch setupTS(Integer tabuListSize, Integer iterations) {
		return new TabuSearch(new StaticTabuList(tabuListSize), new IterationsStopCondition(iterations),
				new BasicNeighborSolutionLocator());
	}
